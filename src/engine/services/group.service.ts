...
const GROUP_PROXIMITY_KM = 0.05; // 50 metres
...
export function resolveGroups(cyclists: Cyclist[]): Group[] {
  ...
  for (const rider of sorted) {
    ...
    groups.push({ id: groupId, cyclistIds: members.map((m) => m.id), speed: groupSpeed });
  }
...
export function getGroupSize(cyclist: Cyclist, cyclists: Cyclist[]): number {
  ...
  return cyclists.filter(
    (c) => c.dynamic.groupId === cyclist.dynamic.groupId,
  ).length;
}
