...
type PrismaRaceWithRelations = PrismaRace & {
  ...
  raceTeams: (PrismaRaceTeam & { team: PrismaTeam & { cyclists: PrismaCyclist[] } })[];
  snapshots?: (PrismaRaceSnapshot & { cyclistSnapshots: (PrismaCyclistSnapshot & { cyclist: PrismaCyclist })[] })[];
};
...
export function toEngineCyclist(dbCyclist: PrismaCyclist): Cyclist {
  return {
    id: dbCyclist.id,
    name: dbCyclist.name,
...
export function toEngineSnapshot(dbSnap: PrismaSnapshotWithChildren): RaceSnapshot {
  return {
    km: dbSnap.km,
...
export function toEngineRace(dbRace: PrismaRaceWithRelations): Race {
  ...
  const cyclists: Cyclist[] = dbRace.raceTeams.flatMap((rt) =>
    rt.team.cyclists
      .filter((c) => rosterIds.size === 0 || rosterIds.has(c.id))
      .map(toEngineCyclist),
  );
  const segments: Segment[] = dbRace.segments.map(toEngineSegment);
  const snapshots: RaceSnapshot[] = (dbRace.snapshots ?? []).map(toEngineSnapshot);
