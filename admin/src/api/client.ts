...
export const api = {
  ...
  getLeaderboard: (id: string) =>
    get<LeaderboardEntry[]>(`/race/${id}/leaderboard`),

  getSeasons: () => get<SeasonSummary[]>("/season"),
  getSeason: (id: string) => get<SeasonDetail>(`/season/${id}`),
  createSeason: (body: { year: number }) =>
    post<SeasonSummary>("/season", body),
  createRace: (body: { name: string; seasonId: string; status?: RaceStatus }) =>
    post<RaceSummary>("/race", body),
  createRaceGroup: (body: {
    name: string;
    seasonId: string;
    stages: { name: string }[];
    status?: RaceStatus;
  }) =>
    post<{ id: string; name: string; seasonId: string; stageCount: number }>(
      "/race-group",
      body,
    ),
  ...
  simulateRace: (raceId: string) =>
    post<RaceSummary>(`/race/${raceId}/simulate`, {}),
};
