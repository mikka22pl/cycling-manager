...
const columns: ColumnDef<SeasonRace, any>[] = [
  ...
  h.accessor('stageNumber', {
    ...
    cell: (info) =>
      info.getValue() != null ? (
        <span className="font-mono text-xs tabular-nums text-gray-500">{info.getValue()}</span>
      ) : (
    ...
    cell: (info) => {
      ...
      return (
        <span className="text-sm text-gray-700">
          {w.name}{' '}
          <span className="text-xs text-gray-400">({w.teamName})</span>
        </span>
      )
    },
  }),
...
export default function SeasonDetailPage() {
  ...
  return (
    <div className="pt-6">
      <div className="mb-2">
        <Link to="/seasons" className="text-xs text-gray-400 hover:text-gray-600">
          ← Seasons
        </Link>
      </div>
      ...
      {isError && (
        <p className="text-sm text-red-400 mt-4">Could not reach the engine API.</p>
      )}
      ...
      {data && (
        <>
          <div className="mb-8 flex items-start justify-between gap-4">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Season {data.year}</h1>
          ...
          {data.races.length === 0 ? (
            <p className="text-sm text-gray-400">No races yet. Create one to get started.</p>
          ) : (
            <DataGrid
              data={data.races}
              columns={columns}
