...
export function decideEffort(
  ...
  // ---- Group context: breakaway = leading group with >0.5 km gap ----
  const groupMembers = allCyclists.filter(
    (c) => c.dynamic.groupId === groupId && !c.dynamic.isDropped,
  );
  const rearOfMyGroup =
    groupMembers.length > 0
      ? Math.min(...groupMembers.map((c) => c.dynamic.position))
      : cyclist.dynamic.position;
  const otherPositions = allCyclists
    .filter((c) => !c.dynamic.isDropped && c.dynamic.groupId !== groupId)
    .map((c) => c.dynamic.position);
  const chaseFront =
    otherPositions.length > 0 ? Math.max(...otherPositions) : -Infinity;
  const inBreakaway =
    groupId !== undefined &&
    groupMembers.length > 0 &&
    chaseFront < rearOfMyGroup - 0.5;
  ...
  effort = clamp(effort + mentalModifier + rng.range(-0.1, 0.1), 0.6, 1.2);
